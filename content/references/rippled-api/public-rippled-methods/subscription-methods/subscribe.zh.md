...
`subscribe`方法在某些事件发生时从服务器请求定期通知。
## Request Format
请求格式的示例:
...
当您订阅一个特定的流时，您会在该流上收到周期性的响应，直到您取消订阅或关闭WebSocket连接为止。这些回复的内容取决于您订阅的内容。这里有一些例子:
...
无论验证消息是否来自受信任的验证器，只要validations流接收到验证消息（也称为验证投票），它就会发送消息。消息如下所示:
...
严格来说，`transactions_proposed`流是`transactions`流的超集：它包括所有已验证的事务，以及一些尚未包含在已验证的分类帐中且可能永远不会包含的建议事务。您可以通过字段识别航班"中的这些"事务:
...
否则，来自`transactions_proposed`流的消息与来自`transactions`流的消息相同。

由于唯一可以修改帐户或订单簿的是事务，因此订阅特定`accounts`或`books`后发送的消息也采用事务消息的形式，与`transactions`流中的消息相同。唯一的区别是，您只接收影响您正在监视的帐户或订单簿的事务的消息。 

`accounts_proposed`订阅的工作方式与此相同，只是它还包括您正在监视的帐户的未确认事务，如`transactions_proposed`流。
...
对等状态流消息表示对等`波及`服务器的状态发生更改的事件。这些消息是带有以下字段的JSON对象:
...
当您使用`books`字段订阅一个或多个订单簿时，您将返回影响这些订单簿的所有事务。
...
订单簿流消息的格式与[transaction stream messages](#transaction-streams) 的格式相同，只是`OfferCreate`事务还包含以下字段:
...
当[the consensus process](consensus.html)更改阶段时，`consenses`流发送`consensespase`消息。该消息包含服务器所处的共识的新阶段。
