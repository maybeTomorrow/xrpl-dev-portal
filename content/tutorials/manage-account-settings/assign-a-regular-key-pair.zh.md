...
XRP分类账允许帐户授权辅助密钥对, 调用一个_[regular key pair](cryptographic-keys.html)_来签署未来的事务。如果普通密钥对的私钥被泄露，您可以删除或替换它，而无需更改[帐户](accounts.html)的其余部分并重新建立它与其他帐户的关系。您还可以主动旋转常规密钥对(对于帐户的主密钥对来说，这两种情况都是不可能的，因为主密钥对与帐户的地址有着内在的联系。）

有关主密钥对和常规密钥对的详细信息，请参阅[加密密钥](cryptographic-keys.html)。

本教程将介绍为帐户分配常规密钥对所需的步骤:
